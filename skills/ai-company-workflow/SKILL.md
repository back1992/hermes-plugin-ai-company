...
**Close contract (TRA-437 incident, 2026-08-15):** Closing an issue MUST go through the tracker's validating API (HTTP endpoint or official CLI) using ONLY statuses the tracker's schema defines - check the tracker's status enum first. Example: ppt-bot-v2's tracker defines `backlog`/`todo`/`in_progress`/`done`/`cancelled`; there is NO `closed` status, so closing = `PATCH {"status": "done"}`. NEVER write status directly to the database/ORM: it bypasses validation and the tracker UI keeps showing the issue as open forever - exactly what happened in TRA-437, when the auto-fix loop ORM-wrote `status="closed"` on three fixed issues. Every close MUST post an evidence comment (fix commit SHA + test/verification output) in the same flow. The outer cycle owns closing; AI Company waves never close issues themselves.

**Companion skills (optional — the reference implementation ppt-bot-v2 uses):**
- An outer coding-cycle skill that owns issue → commit → review → push → close (close contract above: tracker's validating API only, schema-legal statuses, evidence comment - never direct DB/ORM writes); AI Company runs inside its "implement + verify" step.
